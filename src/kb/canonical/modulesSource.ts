...
export interface ModuleSourceEntry {
  mechanic: string
  sections: Record<string, string>
}

export interface ModulesSourceDocument {
  overviewParagraphs: string[]
  deterministicEntries: ModuleSourceEntry[]
}
...
function readModulesStructuredSourceText(): string {
  const sourcePath = getCanonicalSourcePath('modules_structured')
...
function parseDeterministicEntries(lines: string[]): ModuleSourceEntry[] {
  const entries: ModuleSourceEntry[] = []
  let currentEntry: ModuleSourceEntry | null = null
...
export function loadModulesSourceDocument(): ModulesSourceDocument {
  const lines = normalizeLines(readModulesStructuredSourceText())
  const firstMechanicIndex = lines.findIndex(line => line.startsWith('Mechanic: '))

  if (firstMechanicIndex < 0) {
    throw new Error('Modules structured source document is missing mechanic entries.')
  }
