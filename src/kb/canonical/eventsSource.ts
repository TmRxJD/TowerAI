...
export interface EventsSourceDocument {
  overviewParagraphs: string[]
  deterministicEntries: EventSourceEntry[]
}
...
function readEventsStructuredSourceText(): string {
  const sourcePath = getCanonicalSourcePath('events_structured')
...
function parseDeterministicEntries(lines: string[]): EventSourceEntry[] {
  const entries: EventSourceEntry[] = []
  let currentEntry: EventSourceEntry | null = null
...
export function loadEventsSourceDocument(): EventsSourceDocument {
  const lines = normalizeLines(readEventsStructuredSourceText())
  const firstMechanicIndex = lines.findIndex(line => line.startsWith('Mechanic: '))

  if (firstMechanicIndex < 0) {
    throw new Error('Events structured source document is missing mechanic entries.')
  }
