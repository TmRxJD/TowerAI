...
export interface LabsSourceDocument {
  overviewParagraphs: string[]
  deterministicEntries: LabSourceEntry[]
}
...
function readLabsStructuredSourceText(): string {
  const sourcePath = getCanonicalSourcePath('labs_structured')
...
function parseDeterministicEntries(lines: string[]): LabSourceEntry[] {
  const entries: LabSourceEntry[] = []
  let currentEntry: LabSourceEntry | null = null
...
export function loadLabsSourceDocument(): LabsSourceDocument {
  const lines = normalizeLines(readLabsStructuredSourceText())
  const firstMechanicIndex = lines.findIndex(line => line.startsWith('Mechanic: '))

  if (firstMechanicIndex < 0) {
    throw new Error('Labs structured source document is missing mechanic entries.')
  }
